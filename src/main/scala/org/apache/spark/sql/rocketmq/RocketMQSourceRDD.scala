...
private[rocketmq] class RocketMQSourceRDD(
  ...
  override def compute(
    ...
    if (range.fromOffset == range.untilOffset) {
      ...
      val compListenerFunc: (TaskContext) => Unit = (_: TaskContext) => underlying.closeIfNeeded()
      Option( TaskContext.get() ).foreach( _.addTaskCompletionListener[Unit]( compListenerFunc ))
      underlying
    }
  }
